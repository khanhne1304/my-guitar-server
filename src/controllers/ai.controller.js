...
export async function listCloudinaryAudioFiles(req, res, next) {
  try {
    const { folder = 'ai-audio', maxResults = 50, nextCursor } = req.query ?? {};

    const result = await listAudioFilesFromCloudinary(folder, {
    ...
    console.error('❌ Lỗi khi lấy danh sách từ Cloudinary:', error);
    return next(error);
  }
}
